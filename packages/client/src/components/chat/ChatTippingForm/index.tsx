import React, {
  ChangeEventHandler, FC, FormEventHandler, useCallback, useMemo, 
} from 'react';
...
const ChatTippingForm: FC<IProps> = ({
  coin,
  amount,
  balance,
  min,
  ...
  return (
    <form className={classNames(styles.form, formClassName)} onSubmit={onSubmit}>
      <CoinSelect selected={coin} onSelect={onChangeCoin} className={styles.background} />
      ...
      <ChatMinValue min={min} coin={coin} precision={6} />
      ...
      <div className={classNames(styles.total, amountClassName)}>
        <ChatTotalValue total={parseFloat((amount || 0).toFixed(6))} coin={coin} />
      </div>
