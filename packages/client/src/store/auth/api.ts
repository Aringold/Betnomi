...
import {
  ...
  AuthRanksResponse,
  AuthRefreshResponse,
  AuthSignUpResponse, GetGameTokenRequest,
  SetAvatarRequest,
  SetBcCurrency,
  AuthCurencyResponse, DelAvatarRequest,
} from './types';
...
let newCurrency: Promise<any> | undefined;

export const authCurencyChange = async (refresh: string, currency: CoinType) => {
  if (newCurrency) {
    return newCurrency;
  }

  newCurrency = api.post<AuthCurencyResponse>(
    ApiPaths.AuthCurencyChange,
...
export const getNewGameToken = () =>
  api.post<GetGameTokenRequest>(ApiPaths.GameToken);

export const uploadNewAvatar = (file: string) =>
  api.post<SetAvatarRequest>(ApiPaths.ProfileAvatarSet, file);

export const setBcCurrency = (bcCurrency: string) => {
  api.post<SetBcCurrency>(ApiPaths.BcCurrency, { bcCurrency });
};
export const removeAvatar = () =>
  api.delete<DelAvatarRequest>(ApiPaths.ProfileAvatarRemove);
