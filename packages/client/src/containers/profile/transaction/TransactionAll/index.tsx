...
import {
  TransactionAllFormikValues,
  transactionAllInitialValues,
  useTransactionAllFormik,
} from '../../../../hooks/formik/useTransactionAllForm';
...
export const TransactionAll: React.FC<Props> = ({ ClassName }) => {
  ...
  const { isLoading } = useShallowSelector(selectProfileTransactionAll);
  ...
  useEffect(() => {
    dispatch(profileGetTransactionAll(transactionAllInitialValues));
  }, []);
  ...
  const onTimeChange = useCallback((item:Option<string>) => {
    ...
    if (item?.value === TimeRangeType.custom) {
      return showModal(ModalType.SelectDateTransaction)();
    }
