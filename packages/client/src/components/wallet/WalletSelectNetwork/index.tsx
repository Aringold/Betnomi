...
const valueRenderer = (val: Option<CoinType>) => (
  <div className={styles.value}>
    <div className={styles.value__name}>{val.value}</div>
    <div className={styles.value__coin}>{coinNames[val.value]}</div>
  </div>
);
  ...
  return (
    <div className={styles.option}>
      <div className={styles.option__left}>
        <div className={styles.option__name}>{val.value}</div>

        <div className={styles.option__label}>{coinNames[val.value]}</div>
      </div>

      <div className={styles.option__right}>
        <span className={styles.arrival}>{t('Arrival time')}</span>
        {' '}
        <span className={styles.arrival__mins}>
          {t('≈ {{count}} mins', { count: 2 })}
        </span>
      </div>
    </div>
  );
