...
export const SignUpForm: FC<IProps> = ({
  ...
  const { active } = useShallowSelector(selectAffiliateReferralCode);
  ...
  return (
    <form onSubmit={onSubmit}>
      ...
      <div className={styles.referral}>
        <Spoiler openedByDefault={active} title={<span>{t('Promo code (optional)')}</span>}>
          <TextInput
      ...
      >
        <div className={styles.terms_text}>
          {t('I agree to all')}
          <Link to="/terms" className={styles.terms_link} stopPropagation>
            {' '}
            {t('Terms & Conditions')}
            {' '}
          </Link>
