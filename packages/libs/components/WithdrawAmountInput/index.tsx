...
import { truncate } from '@betnomi/client/src/utils/helpers';
...
const WithdrawAmountInput: FC<IProps> = ({
  max, amount, onChangeAmount, coin, ...props 
}) => {
  const { t } = useTranslation('main');

  const onClickMax = useCallback(() => onChangeAmount({ target: { value: truncate(String(max), 10) } } as any), [max, onChangeAmount]);
